...
To run the catalog update automatically after every commit, add a post-commit hook:

```sh
# .githooks/post-commit
#!/usr/bin/env bash
python scripts/update_catalog.py --incremental
```

Then wire it up with `git config core.hooksPath .githooks`.
...
**`.llm/` should be in `.gitignore`** — it contains machine-local state and is not shared:
